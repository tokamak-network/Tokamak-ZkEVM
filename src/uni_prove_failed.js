...
export default async function groth16Prove(cRSName, proofName, QAPName, circuitName, entropy) {
    ...
    for (var i=0; i<s_F; i++){
        ...
        if ( (uX_ki[k] === undefined) ){
            ...
            let {uX_i: uX_i, vX_i: vX_i, wX_i: wX_i} = await polyUtils.readQAP2(QAPName, k, m_k, n, n8r);
            uX_ki[k] = uX_i;
            vX_ki[k] = vX_i;
            wX_ki[k] = wX_i;
            // uX_ki[k][i] is 1-dimentional coefficients array
        }
    }
    ...
    let fY_k = Array.from(Array(s_F), () => new Array(s_max));
    const Fr_s_max_inv = Fr.inv(Fr.e(s_max));
    fY_k[0].fill(Fr_s_max_inv);
    let multiplier = Fr.one;
    for (var k =1; k<s_F; k++){
        multiplier = Fr.mul(multiplier, Fr.inv(omega_y));
        fY_k[k] = fY_k[0].map((i, x) => Fr.mul(x, Fr.exp(multiplier, i)))
    }
    ...
    for(var i=0; i<m; i++){
        ...
        for(var PreImgIdx=0; PreImgIdx<PreImgSize; PreImgIdx++){
            ...
            let u_term = await polyUtils.mulUniPolys2(Fr, uX_ki[s_kPrime][iPrime], fY_k[kPrime]);
            uYX_i = await polyUtils.addPoly(Fr, uYX_i, u_term);

            let v_term = await polyUtils.mulUniPolys2(Fr, vX_ki[s_kPrime][iPrime], fY_k[kPrime]);
            vYX_i = await polyUtils.addPoly(Fr, vYX_i, v_term);

            let w_term = await polyUtils.mulUniPolys2(Fr, wX_ki[s_kPrime][iPrime], fY_k[kPrime]);
            wYX_i = await polyUtils.addPoly(Fr, wYX_i, w_term);
        }
        let term1 = await polyUtils.scalePoly(Fr, uYX_i, cWtns[i]);
        p1XY = await polyUtils.addPoly(Fr, p1XY, term1);
        let term2 = await polyUtils.scalePoly(Fr, vYX_i, cWtns[i]);
        p2XY = await polyUtils.addPoly(Fr, p2XY, term2);
        let term3 = await polyUtils.scalePoly(Fr, wYX_i, cWtns[i]);
