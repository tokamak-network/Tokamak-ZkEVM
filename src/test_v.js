...
export default async function groth16Verify(
  ...
  const proofFile = `${path}/resource/circuits/test_transfer/proof.proof`
  const  circuitReferenceStringFile = `${path}/resource/circuits/test_transfer/test_transfer.crs`
  const circuitDirectory = `${path}/resource/circuits/test_transfer`
  const instanceId = ''
