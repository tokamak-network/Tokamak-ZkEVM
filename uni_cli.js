...
const commands = [
    ...
    {
        cmd: "unisetup [curveName] [power1] [power2] [r1csName] [zkeyName] [entropy]",
        description: "All setups in one",
        alias: ["us"],
        action: uniSetup
    }
];
...
// unisetup [curveName] [power1] [power2] [r1csName] [zkeyName] [entropy]
async function uniSetup(params, options) {
    const curveName = params[0];
    const power1 = params[1];
    const power2 = params[2];
    const r1csName = params[3];
    const zkeyName = params[4];
    const entropy = params[5];

    if (options.verbose) Logger.setLogLevel("DEBUG");

    return zkey.uniSetup(curveName, power1, power2, r1csName, zkeyName, entropy);
}
