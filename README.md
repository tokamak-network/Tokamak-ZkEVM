...
- **buildQAP**
  ...
  - Enter the command "node  build/cli.cjs  QAP_all  bn128  \[s_D]  \[s_max]".
  - Find an output QAP parameter in "./resource/subcircuits/param\_\[s_D]\_\[s_max].dat".
  - Find the output QAPs in "./resource/subcircuits/QAP\_\[s_D]\_\[s_max]/subcircuit#.qap" for all indices # of instructions upto s_D-1.
- **setup**
  - Be sure that the R1CSs generated by compile are in the proper directory.
  - Enter the command "node build/cli.cjs setup param\_\[s_D]\_\[s_max] \[rsName] \[anyNumber]".
...
- **decode**
  ...
  - Copy and paste the output instances into "./resource/circuits/\[circuitName]/instance\[instanceId]/{input_opcode#.json, output_opcode#.json}" for for all indices # of opcodes used in an EVM application less than s_max.
- **derive**
