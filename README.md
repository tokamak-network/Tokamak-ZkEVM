...
- **buildQAP**
  ...
  - Find an output QAP parameter in "./resource/subcircuits/param_$s_D$_$s_{max}$.dat".
  - Find the output QAPs in "./resource/subcircuits/QAP_$s_D$\_$s_{max}$/subcircuit#.qap" for all indices # of instructions upto $s_D-1$.
- **setup**
  - Be sure that the R1CSs generated by compile are in the proper directory.
