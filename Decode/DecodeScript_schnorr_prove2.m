...
code = text2code('schnorr_prove2\bytes_schnorr_prove.txt');
codelen=length(code);
...
fdset1=fopen('schnorr_prove2\Set_I_V.bin', 'w');
fdset2=fopen('schnorr_prove2\Set_I_P.bin', 'w');
fdOpList=fopen('schnorr_prove2\OpList.bin', 'w');
fdWireList=fopen('schnorr_prove2\WireList.bin', 'w');
...
for k=1:length(oplist)
    ...
    fdInput=fopen(['schnorr_prove2\instance\Input_opcode' num2str(k-1) '.json'], 'w');
    fdOutput=fopen(['schnorr_prove2\instance\Output_opcode' num2str(k-1) '.json'], 'w');
